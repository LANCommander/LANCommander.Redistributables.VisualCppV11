...
# Installs the Visual C++ v11 Redistributable (2012) runtime for each selected architecture.
#
...
if ($architecture -eq 'auto') {
    ...
    if ($executable -and (Test-Path -LiteralPath $executable)) {
        ...
        try {
            ...
            # The v14 x64 package carries ARM64 binaries; the 2012 one does not, and
            # Microsoft never shipped an ARM64 build of this runtime at all. x64 is
            # simply the closest thing that exists, and ARM64 Windows runs it under
            # emulation. 0x014C is x86, and is also what a managed AnyCPU executable
            # reports even though it runs 64-bit; those games should be set to Both
            # explicitly.
...
#       Burn sometimes surfaces it as the HRESULT 0x80070666 instead.
...
# The silent-install switches. 2012 takes /install /quiet /norestart, the same as
# 2013 and v14; the 2010 and earlier packages predate that syntax and take
# /q /norestart instead. Passing the wrong set gives a visible installer UI during
# a silent install, or a usage dialog that never returns.
...
foreach ($arch in $required) {
    ...
    Write-Host "Installing Visual C++ v11 Redistributable (2012) ($arch)"
    ...
    if ($acceptable -contains $code) {
        if ($code -eq 1638 -or $code -eq -2147023294) {
            Write-Host "A newer Visual C++ v11 Redistributable (2012) ($arch) is already installed; nothing to do"
        }
        elseif ($code -eq 3010 -or $code -eq 1641) {
            Write-Host "Visual C++ v11 Redistributable (2012) ($arch) installed; a reboot is pending"
        }
